...
def build_features(candles: pd.DataFrame, spy_candles: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    Compute technical indicators from daily OHLC candles.
    No lookahead bias — all features use only past data relative to each row.
    ...
    df[["spy_trend_strength", "spy_price_vs_sma20", "spy_hist_vol_20"]] = (
        df[["spy_trend_strength", "spy_price_vs_sma20", "spy_hist_vol_20"]].ffill()
    )
